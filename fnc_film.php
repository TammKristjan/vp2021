<?php
	...
	function store_film($title_input, $year_input, $duration_input, $genre_input, $studio_input, $director_input) {
		...
		echo $conn->error;
		//seon SQL käsu päris andmetega, andmetüübid :i - integer (täisarv), d - decimal(murdarv), s - string(tekst)
		...
		if($stmt->execute()) {
			...
			$success = "Salvestamine tekkis viga: " .$stmt->error;
			}
		//sulgeme SQL käsu
		$stmt->close();
		//sulgeme andmebaasiühenduse
